...
double _calculateAverageRating(List<Vehicle> vehicles) {
  final vehiclesWithRating = vehicles.where((v) => v.rating != null);
  if (vehiclesWithRating.isEmpty) return 0.0;
  
  final sum = vehiclesWithRating.fold<double>(
    0, (prev, v) => prev + (v.rating ?? 0)
  );
  return sum / vehiclesWithRating.length;
}
